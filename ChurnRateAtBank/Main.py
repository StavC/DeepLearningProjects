...
def main():
    ...
    #### Make the ANN
    hidden_layer_size=0
    output_size=6 #input layer has 11 nodes and in output is 1 so (11+1)/2=6
    model=tf.keras.Sequential()
    model.add(tf.keras.layers.Dense(units=output_size, kernel_initializer='glorot_uniform',activation='relu')) #adding a Layer
    model.add(tf.keras.layers.Dense(units=output_size, kernel_initializer='glorot_uniform',activation='relu')) #adding a Layer
